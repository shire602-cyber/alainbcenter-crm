...
export function TodaysImpact() {
  ...
  if (loading || (!metrics.tasksCompleted && !metrics.messagesSent && !metrics.quotesSent)) {
    return (
      <div className="p-12 text-center rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900/50 dark:to-slate-800/30 border-2 border-slate-200 dark:border-slate-800">
        <div className="w-20 h-20 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center mx-auto mb-4 shadow-lg shadow-slate-500/10">
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        {impactMetrics.map((metric, idx) => {
          ...
          return (
            <Card
              key={idx}
              className="p-5 rounded-xl border-2 border-slate-200 dark:border-slate-800 bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800 shadow-sm hover:shadow-md transition-shadow"
            >
      ...
      {metrics.revenueClosed && metrics.revenueClosed > 0 && (
        <Card className="p-5 rounded-xl border-2 border-green-200 dark:border-green-800 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/10">
